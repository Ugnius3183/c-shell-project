...
# Uses wildcard to grab all c files from src
SRCS = $(wildcard src/*.c)

# Runs gcc compiler using the acquired c files
bin/myshell: $(SRCS)
	mkdir -p bin
	gcc -Wall -o $@ $^
	./bin/myshell

# Removes executable from bin
clean:
	rm -f bin/myshell
	rm -d bin

# Reference:
# 	Oracle, "Using make to compile programs", https://docs.oracle.com/cd/E19504-01/802-5880/6i9k05dhg/index.html (accessed March 17 2023)